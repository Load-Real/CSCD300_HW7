...
public class Hashtable {
    ...
    public Object get(Object key) {
    	if (key == null) throw new NullPointerException("[get() Method]: key parameter CANNOT be null"); //NullPointerException - if the specified key is null
    	
    	int keyLocation = this.hash(key); //This is the location of where the key should be at in the hashtable (first iteration, keyLocation = 19)
    	if (this.table[keyLocation] != null) {
    		HashtableRecord node = new HashtableRecord(); //Create a default HashtableRecord: key = null, value = null
        	node.key = key; //Update the new HashtableRecord's key to the passed in key
        	LinkedList<HashtableRecord> theList = (LinkedList<HashtableRecord>) this.table[keyLocation]; //Create a new LinkedList of Records and set it equal to the Hashtable position of the key
        	HashtableRecord desiredRecord = theList.get(theList.indexOf(node)); //Get the HashtableRecord that is associated with node and return it
    		if (key.equals(node.key)) { //If this map contains a mapping from a key k (node.key) to a value v (Record.value)
    			return desiredRecord.value;
    		}
    		else {
    			return null; //Return null if key k (node.key) does not map to value v (Record.value)
    		}
    	}
        return null; //Return null if no mapping is found in hashtable
    }
